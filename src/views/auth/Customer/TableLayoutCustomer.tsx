    ...
    const tableRows = (rowData: any, index: any) => {
        ...
        const columnData = tableCell.map((key, i) => {
            ...
            if (key === 'is_deleted') {
                return (
                    <td className="text-center" key={i}>
                        {rowData.is_deleted == 1 ? 'Close' : 'Open'}
                    </td>
                )
            }
            if (key === 'index') {
            ...
            if (key === 'status_id') {
                return (
                    <td className="text-center" key={i}>
                        {rowData.status_id === 1 ? 'Pending' : 'Complete'}
                    </td>
                )
            }
            if (key === 'asset_type_id') {
                return (
                    <td className="text-center" key={i}>
                        {rowData.asset_type_id === 1
                            ? 'Store'
                            : rowData.asset_type_id == 2
                            ? 'Move'
                            : 'Prepare'}
                    </td>
                )
            ...
            if (key === 'Action') {
                return (
                    <td className="flex justify-center p-1 gap-3" key={i}>
                        ...
                        <Button
                            className="!p-2"
                            disabled={rowData?.is_deleted === 1}
                            onClick={() =>
                                handleDelete(rowData?.master_query_id)
                            }
                        >
                            Close
                        </Button>
        ...
        <div className="shadow-2xl p-4">
            ...
            <div className="overflow-auto">
                <table className="w-[100%]">
                    <thead className="">
                        <tr className="bg-black text-white det-header rounded-[13px] my-2 h-[40px]">
                            {headRow()}
                        </tr>
                    </thead>
