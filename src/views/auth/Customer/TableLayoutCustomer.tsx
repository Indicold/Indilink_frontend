...
const TableLayoutCustomer = ({ AllStore, refetchAgain }: any) => {
  ...
  const tableRows = (rowData: any, index: any) => {
    ...
    const columnData = tableCell.map((key, i) => {
      ...
      if (key === 'status_id') {
        return <td className='text-center' key={i} >{rowData.status_id === 1 ? "Pending" : "Complete"}</td>;
      }
      if (key === 'asset_type_id') {
        return <td className='text-center' key={i} >{rowData.asset_type_id === 1 ? "Store" : rowData.asset_type_id == 2 ? "Move" : "Prepare"}</td>;
      }
      if (key === 'contract_name') {
