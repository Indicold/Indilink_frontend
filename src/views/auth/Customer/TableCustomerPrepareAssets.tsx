  ...
  const tableRows = (rowData: any, index: any) => {
    ...
    const columnData = tableCell.map((key, i) => {
      ...
      if (key === 'Action') {
        ...
        <Button className='!p-3 pt-0 pb-0' disabled={rowData?.is_accepted===1 || rowData?.is_accepted===2} onClick={() => handleAccept(rowData)}>Accept</Button>
        <Button className='!p-2' disabled={rowData?.is_accepted===2 || rowData?.is_accepted===1} onClick={() => handleReject(rowData)}>Reject</Button>
