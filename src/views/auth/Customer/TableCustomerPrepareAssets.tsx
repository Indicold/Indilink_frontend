...
// The TableCustomerPrepareAssets component takes a prop called AllStore, presumably for rendering data.

const TableCustomerPrepareAssets = ({ AllStore, fetchAgain }: any) => {
    ...
    const handleEdit = (rowData: any) => {
        ...
        if (rowData?.asset_type_id == 3) {
            navigate('/customer-prepare', {
                state: { data: rowData, disabled: false, extraForm: true },
            })
        }
    ...
    const handleDocs = (rowData: any) => {
        if (rowData?.asset_type_id == 3) {
            navigate(`/customer-documents-list/${rowData?.master_query_id}`, {
                state: { data: rowData, disabled: true, extraForm: true },
            })
        }
    }
    const handleView = (rowData: any) => {
        if (rowData?.asset_type_id == 3) {
            navigate(`/assetsprepare-details/${rowData?.asset_id}`, {
                state: { data: rowData, disabled: true, extraForm: true },
            })
        }
        if (rowData?.asset_type_id == 2) {
            navigate(`/assetsmove-details/${rowData?.asset_id}`, {
                state: { data: rowData, disabled: true, extraForm: true },
            })
        }
        if (rowData?.asset_type_id == 1) {
    ...
    const tableRows = (rowData: any, index: any) => {
        ...
        const columnData = tableCell.map((key, i) => {
            ...
            if (key === 'Action') {
                return (
                    <td className="text-center flex gap-2 p-1" key={i}>
                        {GetNdaStatusData?.data[0]?.is_nda_signed == 2 && (
                            <Button
                                className="!p-3 m-auto pt-0 pb-0"
                                disabled={
...
export default TableCustomerPrepareAssets
