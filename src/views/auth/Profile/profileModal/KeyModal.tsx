...
const KeyModal = ({ data, setData, modal, setModal, fetchData }: any) => {
    ...
    const {
        ...
        `master/get-state-by-countryId/${data?.country_id}`,
        token
    )
    ...
    let {
        result: Keyesponse,
        loading: KeyLoading,
        sendPostRequest: KeyPostDetails,
    }: any = usePostApi(`auth/key-mgmt`)
    let {
        result: KeyUpdateesponse,
        loading: KeyUpdateLoading,
        sendPostRequest: KeyUpdatePost,
    }: any = usePutApi(`auth/key-mgmt/${data?.id}`)
    ...
    const handleChange = (e: any) => {
        ...
        if (e.target.name === 'aadhar') {
            ...
            setData(newdata)
            if (error[e.target.name])
                validateKeyForm(newdata, setErrors, isEmailValid, isMobileValid)
        }
    }
    console.log('TTYYYYY9999', isEmailValid)

    const handlesubmit = () => {
        console.log(
            'TYYYYYYYYRR',
            validateKeyForm(data, setErrors, isEmailValid, isMobileValid),
            error
        )
