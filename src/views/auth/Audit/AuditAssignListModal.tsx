...
const AuditAssignListModal = ({ setModal, assetData }: any) => {
    ...
    const { data: AuditAssignList } = useApiFetch2<any>(
        `audit/auditModulesss/${id}`,
        token.token
    )
    const { data: gradeByID } = useApiFetch2<any>(
        `audit/audit_grade/${gradeID}`,
        token.token
    )
    const {
        result: AddAssetOrderResponse,
        loading,
        sendPostRequest,
    }: any = usePostApi('audit/addAssetOrder')
                ...
                <div className="relative w-full p-2 my-6 mx-auto lg:w-[800px] my-auto ">
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        ...
                        <div className="relative">
                            {AuditAssignList?.data?.length > 0 ? (
                                <AuditAssignTableList
                                    allData={AuditAssignList?.data}
