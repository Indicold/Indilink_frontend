...
const AuditAssignTableList = ({setType, allData, setModal, setAssetId, setDisabled, setData, fetchStoreDataAgain,headcolor,handleRedirect }: any) => {
    ...
    let { data: userList, loading: ULLoading }: any = useApiFetch2(`admin/users`, token);
    ...
    const [collection, setCollection] = React.useState(allDataArray
  ...
  */
    ...
    const tableRows = (rowData: any, index: any) => {
        ...
        const columnData = tableCell.map((key, i) => {
            ...
            if (key === 'id') {
                return (<td className=' flex justify-center p-2 gap-3' key={i} >{(currentPage-1) * countPerPage + index + 1}</td>)
            }
            if (key === 'status') {
                return (<td className=' flex justify-center p-2 gap-3' key={i} >{rowData?.status==0 ? "Pending":rowData?.status==1 ? "Accept":"Rejected"}</td>)
            }
            if (key === 'is_verified') {
                return (<td className=' flex justify-center p-2 gap-3' key={i} >{rowData?.is_verified === 1 ? "Verified" : "Not Verified"}</td>)
            }
            if(key=== 'update_by'){
                
              return ( <td  key={i} >
                   {`${userList?.filter((item:any,index:any)=>item?.id==rowData?.update_by)[0]?.first_name} 
                   ${userList?.filter((item:any,index:any)=>item?.id==rowData?.update_by)[0]?.last_name!=null ? userList?.filter((item:any,index:any)=>item?.id==rowData?.update_by)[0]?.last_name :"" }` }
            ...
            if(key=== 'created_by'){
              ...
              return ( <td  key={i} >
                {`${userList?.filter((item:any,index:any)=>item?.id==rowData?.created_by)[0]?.first_name} ${userList?.filter((item:any,index:any)=>item?.id==rowData?.created_by)[0]?.last_name !=null ? userList?.filter((item:any,index:any)=>item?.id==rowData?.created_by)[0]?.last_name :""}` }
            ...
            if (key == 'Action') {
                return (
                    <div >
                         <td className='flex justify-center p-2 gap-3' key={rowData.id}>
        <Button className='!p-3 pt-0 pb-0' onClick={() => handleRedirect(rowData)}>
          <VisibilityIcon />
        </Button>
  {/* {canViewLog && (
        <Button className='!p-3 pt-0 pb-0' onClick={() => handleLogs(rowData)}>
          Logs
        </Button>
      )} */}
      {/* {canView && (
      ...
      {/* Add more conditions for other buttons with icons */}
            ...
            return <td key={i} className='p-3'>{rowData[key]}</td>;
    ...
    const tableData = () => {
        ...
        return collection?.length>0 ? collection?.map((rowData: any, index: any) => tableRows(rowData, index)) :<tr>
          <td colSpan={12}><h4>Data Not Found</h4></td>
        </tr>;
    ...
    const headRow = () => {
        ...
        return Object.values(tableHead).map((title, index) => {
            if (title === "Action") {
                if (permission?.role === 'Add Assets' || permission?.role === 'Edit Assets' || permission?.role === 'View Assets') {
                    return <td className="p-2 text-center" key={index} >{title}</td>
    ...
    return (
<>
{AuditModal && <AuditLogsModal setModal={setAuditModal} RowData={RowData} />}
     {true ?
        <>
            <ToastContainer />
...
<div className='table-container'>
            <table className='w-[100%]'>
                ...
                <thead className=' w-[100%]'>
                    ...
                    <tr className={`${headcolor ? headcolor :"bg-table-column"} text-white det-header rounded-[13px] my-2 h-[40px]`}>{headRow()}</tr>

                </thead>

                <tbody className="trhover bg-white text-center">{tableData()}</tbody>
